[CmdletBinding()]
param(
    [string]$DatabaseUrl = $(if ($env:SIGMA_DB_URL) { $env:SIGMA_DB_URL } else { $env:TG_DB_URL }),
...
function Get-PgConnectionInfo {
    ...
    if ($normalized -notmatch '^postgresql://(?<user>[^:]+):(?<password>[^@]*)@(?<host>[^:/?]+)(:(?<port>\d+))?/(?<db>[^?]+)') {
        throw "Не удалось разобрать DatabaseUrl: $Url"
    }
...
if (-not $DatabaseUrl) {
    throw "DatabaseUrl не задан. Заполните SIGMA_DB_URL или TG_DB_URL."
}
...
$cleanupFile = Join-Path $scriptsDir "030_cleanup_7_days.sql"
...
Invoke-PsqlFile -Database $conn.Database -FilePath $cleanupFile

Write-Host "Sigma cache database objects are ready: $($conn.Database)"
